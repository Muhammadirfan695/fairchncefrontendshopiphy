const asyncHandler = require('backend/middleware/node_modules/express-async-handler');
var path = require('path');
const { successResponse, PrintError } = require('../middleware/common');
const AWS = require('backend/middleware/node_modules/aws-sdk');
const fs = require('fs');
  ...
  S3.putObject(params, (err, response) => {
    if (err) {
      ...
      return successResponse(
        200,
        'added successfully',
        { fileUrl: `${process.env.IMAGEBASEURLAWS}` + imagenamefinal, fileName: imagenamefinal },
        res
      );
    }
  });
